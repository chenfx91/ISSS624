...
Notice there is a region with no links (i.e. region with index number 86), we need to remove it and re-run the above 2 code chunks.
...
```{r}
lcosts <- nbcosts(nga_derived.nb, cluster_vars)
```
...
```{r}
clust6 <- spdep::skater(edges = nga_derived.mst[,1:2], 
                 data = cluster_vars, 
                 method = "euclidean", 
                 ncuts = 5)
```
...
The boxplot reveals Cluster 3 has the highest mean non-functional water points in the district. This is followed by Cluster 6, 5, 4, 2 and 1.
