...
From the above graphs, we can see that in the northern part of the country, we can see more functional water points while in the central and east sides, there are more non-functional water points.
...
The code chun below show the steps to prepare a LISA cluter map.
...
```{r}
longtitude_wp <- map_dbl(nga_wp$geometry,~st_centroid(.x)[[1]])
latitude_wp <- map_dbl(nga_wp$geometry,~st_centroid(.x)[[2]])
coords_wp <- cbind(longtitude_wp,latitude_wp)
```
...
k1dists_wp <- unlist(nbdists(k1_wp,coords_wp, longlat = TRUE))
summary(k1dists_wp)
```
...
```{r}
wm_d72 <- dnearneigh(coords_wp, 0, 72, longlat = TRUE)
wm_d72
```
...
```{r}
gi.fixed_wp <- localG(nga_wp$`wpt non-functional`,wm72_lw)
gi.fixed_wp
```
...
```{r}
gi.adaptive_wp <- localG(nga_wp$`wpt non-functional`,wm72_lw)
