---
title: "In-class_Ex4: Calirating Hedonic Pricing Model for Private Highrise Property with GWR Method"
editor: visual
---

## Overview

**Geographically weighted regression (GWR)** is a spatial statistical technique that takes non-stationary variables into consideration (e.g., climate; demographic factors; physical environment characteristics) and models the local relationships between these independent variables and an outcome of interest (also known as dependent variable). In this hands-on exercise, you will learn how to build [hedonic pricing](https://www.investopedia.com/terms/h/hedonicpricing.asp) models by using GWR methods. The dependent variable is the resale prices of condominium in 2015. The independent variables are divided into either structural and locationa.
...
-   R package for calibrating geographical weighted family of models
    ...
    -   [**GWmodel**](https://cran.r-project.org/web/packages/GWmodel/)(this package can also do predictive model)
...
corrplot(cor(condo_resale[, 5:23]), diag = FALSE, order = "AOE",
         tl.pos = "td", tl.cex = 0.5, method = "number", type = "upper") #note here do not use condo_resale.sf as it contains geometry fields. in R is sensitive to the geometry objects.
```
...
condo.mlr <- lm(formula = SELLING_PRICE ~ AREA_SQM + AGE    + 
                ...
                data=condo_resale.sf) #note that here use the sf table
summary(condo.mlr)
```
...
With the below results, we look at the Adjusted R2, we can conclude that the model can address 65% of the price variations.

Looking at Anova, since sigma is small, we can reject the H0 and conclude that...
...
bw.fixed <- bw.gwr(formula = SELLING_PRICE ~ AREA_SQM + AGE + PROX_CBD + 
                   ...
                   approach="CV", #cv is cross validation
                   kernel="gaussian", 
                   adaptive=FALSE, 
                   longlat=FALSE) #True: data in decimals. false: means input data itself is already projected
```

The result shows that the recommended bandwidth is 971.3405 metres (from the last number).(Quiz: Do you know why it is in metre? A: due to the svy21 coordinate system.)
...
Compare the AICc for GW and Non GW, then we know that GW model is better. how much improvement? refer to the adjusted R2, 20% better for GW model.
